...
export const signUp = async (req,res) => {
    ...
    try {
        //first check if all the information is collected
        if (!fullName || !email || !password || !bio) {
            return res.json({success: false, message: "Missing Details"})
        }
        
        //check if the email already be registered
...
export const updateProfile = async (req, res) => {
    try {
        ...
        if (!profilePic) {
            ...
            const upload = await cloudinary.uploader.upload(profilePic); //update to the cloudinary
